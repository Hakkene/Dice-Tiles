...
const MainScreen = () => {
  ...
  useEffect(() => {
    const fetchDataNew = async () => {
      try {
        const url = `http://www.diceandtiles.xyz/api/products/?page=${currentPageNew}&page_size=5&sort_by=id_desc`;
  ...
  useEffect(() => {
    const fetchDataHot = async () => {
      try {
        const url = `http://www.diceandtiles.xyz/api/products/?page=${currentPageHot}&page_size=5&sort_by=upvotes`;
        const response = await fetch(url);
        const data = await response.json();
